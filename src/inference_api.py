...
class ChatCompletionRequest(BaseModel):
    model: str = Field(default=INFERENCE_MODEL_NAME, description="Model identifier")
...
inference_app = FastAPI(title="AI Inference Server", version="1.0.0")
...
async def list_models():
    ...
    return {
        ...
        "data": [
            {
                "id": INFERENCE_MODEL_NAME,
                "object": "model",
                "created": int(time.time()),
                "owned_by": "local",
                "permission": [],
                "root": INFERENCE_MODEL_NAME,
                "parent": None
            }
        ]
    }
